...
def read_x_y(path: str, target_name: str):
...
def print_model_results(y_train, y_pred_train, y_test, y_pred_test):
    ...
    print("Training: \nRMSE: ", rmse_train, " | MAE: ", mean_absolute_error(
        y_train, y_pred_train), " | R^2: ", r2_score(y_train, y_pred_train), "\n")
    print("Test: \nRMSE: ", rmse_test, " | MAE: ", mean_absolute_error(
        y_test, y_pred_test), " | R^2: ", r2_score(y_test, y_pred_test), "\n")
...
def plot_pvi_boxplots(pvi, n, data, model_name, img_path):
    ...
    plt.grid()
    plt.boxplot(pvi.importances[pvi_sorted_idx][-n:].T,
                vert=False, labels=data.columns[pvi_sorted_idx][-n:])
